...
export class EncounterBuilderUiBestiary extends EncounterBuilderUi {
	...
	render () {
		const rdState = super.render({
			stgSettings: veEs("#wrp-encounterbuild-settings"),
			stgRandomAndAdjust: veEs("#wrp-encounterbuild-random-and-adjust"),
			stgShapeCustom: veEs("#wrp-encounterbuild-shape-custom"),
			stgGroup: veEs("#wrp-encounterbuild-group"),
			stgDifficulty: veEs("#wrp-encounterbuild-difficulty"),
		});
	...
	_doUpdateDispSummary () {
		...
		this._dispSummary.vee.html(`${monCount} creature${monCount === 1 ? "" : "s"}; ${this._getActiveRulesComp().getDisplaySummary()}`);
	}
	...
	async _pDoCrChange ({iptCr, monScaled, scaledTo}) {
		...
		const targetCr = UiUtil.strToCr(iptCr.vee.val());

		if (targetCr == null) {
			JqueryUtil.doToast({
				content: `"${iptCr.vee.val()}" is not a valid Challenge Rating! Please enter a valid CR (0-30).`,
				type: "danger",
			});
			iptCr.vee.val(Parser.numberToCr(scaledTo ?? baseCrNum));
			return;
		}
	...
	getSublistButtonsMeta (sublistItem) {
		const btnAdd = veT`<button title="Add (SHIFT for 5)" class="ve-btn ve-btn-success ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-plus"></span></button>`
			.vee.onn("click", evt => this._handleClick({evt, entity: sublistItem.data.entity, mode: "add"}));

		const btnSub = veT`<button title="Subtract (SHIFT for 5)" class="ve-btn ve-btn-danger ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-minus"></span></button>`
			.vee.onn("click", evt => this._handleClick({evt, entity: sublistItem.data.entity, mode: "subtract"}));

		const btnRandomize = veT`<button title="Randomize Monster" class="ve-btn ve-btn-default ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-random"></span></button>`
			.vee.onn("click", evt => this._pHandleShuffleClick({evt, sublistItem}));

		const btnLock = veT`<button title="Lock Monster against Randomizing/Adjusting" class="ve-btn ve-btn-default ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-lock"></span></button>`
			.vee.onn("click", () => this._sublistManager.pSetDataEntry({sublistItem, key: "isLocked", value: !sublistItem.data.isLocked}))
			.vee.toggleClass("ve-active", sublistItem.data.isLocked);

		const wrp = veT`<span class="best-ecgen__visible ve-col-1-5 ve-no-wrap ve-pl-0 ve-btn-group">
			${btnAdd}
			${btnSub}
			${btnRandomize}
			${btnLock}
		...
		return {
			wrp,
			fnUpdate: () => btnLock.vee.toggleClass("ve-active", sublistItem.data.isLocked),
		};
	}
}
